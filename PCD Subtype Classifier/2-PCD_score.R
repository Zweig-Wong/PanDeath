...
PCD_Subtype <- fread("Output/1-Infer_PCD_subtype/Prediction.tsv")[, .(ID_sample, PCD_Subtype = `PCD Subtype`)]
...
PCD_Expr <- transpose(PCD_Expr, keep.names = "Gene", make.names = "ID_sample") %>% 
    ...
    as.matrix() 
PCD_Gene_list <- list()
for (name in unique(PCD_Gene[["PCD_Mode"]])) {
    PCD_Gene_list[[name]] <- intersect(PCD_Gene[name, on = "PCD_Mode"]$Gene, rownames(PCD_Expr))
}
...
## Plot
for (Subtype in c("PCD-P", "PCD-PN", "PCD-NF", "PCD-FA", "PCD-A")) {
